...
```{r}
library(readr)
counts <- read.csv("~/RNAseq_Anoikis/OV90/RNAseq/15U/counts/counts_15U_only.csv")
View(counts)
```

```{r}
no_decimals <- gsub('\\.[0-9]*$','',counts$X)
colnames(counts) <- c("X", "U01", "U02", "U03", "U11", "U12", "U13", "U111", "U112", "U113")
...
```{r}
write.csv(gsva.es, "~/RNAseq_Anoikis/OV90/RNAseq/15U/15U_only/OV90_15U_only_Hallmarks_res_gsva.csv")
```
