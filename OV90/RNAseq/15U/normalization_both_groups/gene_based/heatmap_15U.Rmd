...
So now we have a data frame for each up and down regulated set in each seq data set Now, we want to combine these and use the information to make a heatmap.
...
```{r}
combined$P0_P1_DMSO_LFC <- as.numeric(combined$P0_P1_DMSO_LFC)
combined$P0_15U_LFC <- as.numeric(combined$P0_15U_LFC)
combined$P1_15U_LFC <- as.numeric(combined$P1_15U_LFC)
combined$P11_15U_LFC <- as.numeric(combined$P11_15U_LFC)
mat <- as.data.frame(combined)
...
```{r}
#write.csv(mat_3, "~/Desktop/mat_3.csv", row.names= FALSE)
```
...
```{r}
mat_3 <- as.matrix(read.csv("~/Desktop/mat_3.csv"))
```

```{r}
library(ComplexHeatmap)
Heatmap(mat_3, col=coul2, cluster_columns = TRUE, use_raster= TRUE)
```
```{r}
ggplot(data = combined, aes(x = P0_15U_LFC, y = P1_15U_LFC)) + geom_point(alpha = 1, size=1) + geom_vline(xintercept = 0, 
                color = "black", linewidth=1.5) + geom_hline(yintercept=0, color = "black", linewidth=1.5) + theme(legend.key.size = unit(3, 'cm'), legend.text = element_text(size=19), legend.title=element_blank()) + guides(color=guide_legend(ncol=2)) + guides(shape=guide_legend(ncol=2)) + theme(panel.background = element_blank(), panel.grid.major= element_line()) + theme(axis.line = element_line(colour = "black"))
ggsave("~/Desktop/P0_P1_15U.tiff", plot = last_plot()) 
```
```{r}
ggplot(data = combined, aes(x = P0_15U_LFC, y = P0_P1_DMSO_LFC)) + geom_point(alpha = 1, size=1) + geom_vline(xintercept = 0, 
                color = "black", linewidth=1.5) + geom_hline(yintercept=0, color = "black", linewidth=1.5) + theme(legend.key.size = unit(3, 'cm'), legend.text = element_text(size=19), legend.title=element_blank()) + guides(color=guide_legend(ncol=2)) + guides(shape=guide_legend(ncol=2)) + theme(panel.background = element_blank(), panel.grid.major= element_line()) + theme(axis.line = element_line(colour = "black"))
ggsave("~/Desktop/P0_15U_vs_P0_P1_DMSO.tiff", plot = last_plot()) 
```

```{r}
ggplot(data = combined, aes(x = P1_15U_LFC, y = P0_P1_DMSO_LFC)) + geom_point(alpha = 1, size=1) + geom_vline(xintercept = 0, 
                color = "black", linewidth=1.5) + geom_hline(yintercept=0, color = "black", linewidth=1.5) + theme(legend.key.size = unit(3, 'cm'), legend.text = element_text(size=19), legend.title=element_blank()) + guides(color=guide_legend(ncol=2)) + guides(shape=guide_legend(ncol=2)) + theme(panel.background = element_blank(), panel.grid.major= element_line()) + theme(axis.line = element_line(colour = "black"))
ggsave("~/Desktop/P1_15U_vs_P0_P1_DMSO.tiff", plot = last_plot()) 
```

```{r}
ggplot(data = combined, aes(x = P11_15U_LFC, y = P0_P1_DMSO_LFC)) + geom_point(alpha = 1, size=1) + geom_vline(xintercept = 0, 
                color = "black", linewidth=1.5) + geom_hline(yintercept=0, color = "black", linewidth=1.5) + theme(legend.key.size = unit(3, 'cm'), legend.text = element_text(size=19), legend.title=element_blank()) + guides(color=guide_legend(ncol=2)) + guides(shape=guide_legend(ncol=2)) + theme(panel.background = element_blank(), panel.grid.major= element_line()) + theme(axis.line = element_line(colour = "black"))
ggsave("~/Desktop/P11_15U_vs_P0_P1_DMSO.tiff", plot = last_plot()) 
```
