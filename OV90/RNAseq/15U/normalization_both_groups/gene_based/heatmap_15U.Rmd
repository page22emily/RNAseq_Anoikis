...
```{r}
df <- data.frame(counts_D1_averages)
```
...
df$LC_1 <- foldchange(df$P01, df$counts_D1_averages) 
df$D1A <- foldchange2logratio(df$LC_1, base = 2)
df$LC_2 <- foldchange(df$P02, df$counts_D1_averages)
df$D1B<- foldchange2logratio(df$LC_2, base = 2)
df$LC_3 <- foldchange(df$P03, df$counts_D1_averages)
...
```{r}
df <- data.frame(counts_D11_average)
```
...
df$LC_1 <- foldchange(df$P01, df$counts_D11_average) 
df$D11A <- foldchange2logratio(df$LC_1, base = 2)
df$LC_2 <- foldchange(df$P02, df$counts_D11_average)
df$D11B<- foldchange2logratio(df$LC_2, base = 2)
df$LC_3 <- foldchange(df$P03, df$counts_D11_average)
...
```{r}
mat_num <- mat[ , c(1:4, 11:13, 5:7, 14:16, 8:10, 17:19)]
...
x <- data.frame(counts_U1_average)
...
```{r}
df2$LC_1 <- foldchange(df2$counts_U0_average, df2$counts_D0_average) 
df2$P0_DMSO_vs_15U <- foldchange2logratio(df2$LC_1, base = 2)
df2$LC_2 <- foldchange(df2$counts_U1_average, df2$counts_D1_average) 
df2$P1_DMSO_vs_15U <- foldchange2logratio(df2$LC_2, base = 2)
df2$LC_3 <- foldchange(df2$counts_U11_average, df2$counts_D11_average)
df2$P11_DMSO_vs_15U <- foldchange2logratio(df2$LC_3, base = 2)
```
...
```{r}
write.csv(mat3, "~/Desktop/lfc_averages_15U_comparison_DMSO_15U_averages.csv")
```

```{r}
library(readr)
mat4<- read_csv("~/Desktop/lfc_averages_15U_comparison_DMSO_15U_averages.csv", 
    col_types = cols(...1 = col_number(), 
       P0_DMSO_vs_15U = col_number(), P1_DMSO_vs_15U = col_number(), 
        P11_DMSO_vs_15U = col_number()))
```
...
```{r}
pdf("~/Desktop/heatmap_15U_DMSO_15U_comparison_DMSO_15U_averages.pdf",width=8, height=11)
