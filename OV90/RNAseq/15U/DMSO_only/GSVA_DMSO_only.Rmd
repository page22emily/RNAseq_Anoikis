...
```{r}
library(readr)
counts <- read.csv("~/RNAseq_Anoikis/OV90/RNAseq/15U/counts/counts_DMSO_only.csv")
View(counts)
```

```{r}
no_decimals <- gsub('\\.[0-9]*$','',counts$X)
colnames(counts) <- c("X", "D01", "D02", "D03", "D11", "D12", "D13", "D111", "D112", "D113")
...
```{r}
write.csv(gsva.es, "~/RNAseq_Anoikis/OV90/RNAseq/15U/DMSO_only/OV90_DMSO_only_Hallmarks_res_gsva.csv")
```
