...
P0_P3 <- read.csv("~/Desktop/OV90_Outputs/DESeq2/P3_vs_P0.csv")
...
P0_P3_adj2 <- P0_P3_adj[P0_P3_adj$padj <= .05 & P0_P3_adj$log2FoldChange >= 1.5, ]
P0_P3_adj3 <- P0_P3_adj[P0_P3_adj$padj <= .05 & P0_P3_adj$log2FoldChange <= -1.5, ]
...
```{r}
sigGenes <- as.list(c(P0_P1_adj2$X, P0_P1_adj3$X, P0_P4_adj2$X, P0_P4_adj3$X, P0_P7_adj2$X, P0_P7_adj3$X, P0_P6_adj2$X, P0_P6_adj3$X, P0_P3_adj2$X, P0_P3_adj3$X))
...
```{r}
counts_P3 <- counts[ , 8:10]
rownames(counts_P3) <- counts$X
colnames(counts_P3) <- c("One", "two", "three")
counts_P3 <- as.data.frame(counts_P3)
counts_P3_averages <- rowMeans(counts_P3)
```
...
```{r}
df$P01 <- counts_P3$One
df$P02 <- counts_P3$two
df$P03 <- counts_P3$three
df$LC_1 <- foldchange(df$P01, df$counts_P0_average) 
df$P3A <- foldchange2logratio(df$LC_1, base = 2)
df$LC_2 <- foldchange(df$P02, df$counts_P0_average)
df$P3B<- foldchange2logratio(df$LC_2, base = 2)
df$LC_3 <- foldchange(df$P03, df$counts_P0_average)
df$P3C <- foldchange2logratio(df$LC_3, base = 2)
```
...
```{r}
combined <- rbind(de21, de2, de25, de22, de23, de24)
```
...
```{r}
combined4 <- merge(deseq2VST_wide01, deseq2VST_wide1, all.x=TRUE, all= TRUE)
combined3 <- merge (combined4, deseq2VST_wide15, all.x=TRUE, all= TRUE)
combined1 <- merge(combined3, deseq2VST_wide4, all.x=TRUE, all= TRUE)
...
combined_redo <- read_csv("~/Desktop/combined.csv", 
    col_types = cols(...1 = col_number(), 
        ...
        P1B = col_number(), P1C = col_number(), P3A = col_number(), 
        P3B = col_number(), P3C = col_number(), 
...
pdf("~/Desktop/heatmap_Ov90_Gene.pdf",width=8, height=11)
