...
P0_vs_P3 <- read.csv("~/Desktop/OV90_Outputs/DESeq2/P3_vs_P0.csv")
...
no_decimals <- gsub('\\.[0-9]*$','', P0_vs_P3$X)
P0_vs_P3$X <- no_decimals
...
P0_P3_adj2 <- P0_P3_adj[P0_P3_adj$padj <= .05, ]
...
```{r}
P0_P3_Annotations <- add_column(P0_P3_adj2 %>%
  mutate(Gene_Annotation = case_when(log2FoldChange <= -1.5 ~ "Down Regulated", log2FoldChange >= 1.5 ~ "Up Regulated", between(log2FoldChange, -1.5 , 1.5) ~ "No Change")))
P0_P3_Annotations <- add_column(P0_P3_Annotations %>%
  mutate(Down = if_else(log2FoldChange <= -1.5, TRUE, FALSE), .after="Gene_Annotation"))
P0_P3_Annotations <- add_column(P0_P3_Annotations %>%
  mutate(Up = if_else( log2FoldChange >= 1.5, TRUE, FALSE), .after="Down"))
P0_P3_Annotations <- add_column(P0_P3_Annotations %>%
...
P3_Up <- P0_P3_Annotations[ , c(1,10)]
P3_Down <- P0_P3_Annotations[ , c(1,9)]
P3_NC <- P0_P3_Annotations[ , c(1,11)]
...
colnames(P3_Up) <- c("gene_id", "P0_vs_P3")
colnames(P3_Down) <- c("gene_id", "P0_vs_P3")
colnames(P3_NC) <- c("gene_id", "P0_vs_P3")
...
```{r}
Up_Genes1 <- merge(P1_Up, P3_Up, all.x=TRUE, all= TRUE)
Up_Genes2 <- merge(Up_Genes1, P4_Up, all.x=TRUE, all= TRUE)
Up_Genes3 <- merge(Up_Genes2, P6_Up, all.x=TRUE, all= TRUE)
Up_Genes <- merge(Up_Genes3, P7_Up, all.x=TRUE, all= TRUE)
```

```{r}
Down_Genes1 <- merge(P1_Down, P3_Down, all.x=TRUE, all= TRUE)
Down_Genes2 <- merge(Down_Genes1, P4_Down, all.x=TRUE, all= TRUE)
Down_Genes3 <- merge(Down_Genes2, P6_Down, all.x=TRUE, all= TRUE)
Down_Genes<- merge(Down_Genes3, P7_Down, all.x=TRUE, all= TRUE)
```

```{r}
NC_Genes1 <- merge(P1_NC, P3_NC, all.x=TRUE, all= TRUE)
NC_Genes2 <- merge(NC_Genes1, P4_NC, all.x=TRUE, all= TRUE)
NC_Genes3 <- merge(NC_Genes2, P6_NC, all.x=TRUE, all= TRUE)
NC_Genes<- merge(NC_Genes3, P7_NC, all.x=TRUE, all= TRUE)
```
...
```{r}
write.csv(up, "~/Desktop/up_genes_shared_OV90.csv")
write.csv(down, "~/Desktop/down_genes_shared_OV90.csv")
write.csv(nc, "~/Desktop/nc_genes_shared_OV90.csv")
```
...
ggsave("~/Desktop/Up_OV90_fullset.pdf", last_plot())
```
...
ggsave("~/Desktop/Down_OV90_fullset.pdf", last_plot())
```
...
ggsave("~/Desktop/NC_OV90_fullset.pdf", last_plot())
```
...
```{r}
upset(up, Group, min_degree=1, sort_sets= FALSE, sort_intersections_by='degree', base_annotations= list('Size'=(intersection_size(counts=FALSE))), sort_intersections= "ascending") + ggtitle('Up Regulated Genes') 
ggsave("~/Desktop/Up_OV90_fullset_no_labels.tiff", last_plot())
```
...
ggsave("~/Desktop/Down_OV90_fullset_no_labels.tiff", last_plot())
```
...
ggsave("~/Desktop/NC_OV90_fullset_no_labels.tiff", last_plot())
```
