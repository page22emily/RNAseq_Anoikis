...
```{r message= FALSE, warning= FALSE, echo=FALSE}
sample_meta_data <- read.csv(file= "/Users/emilypage/Documents/Mythreye_Lab/Anoikis_Project/Bulk_RNA/OV90/DESeq2/meta_data_anoikis.csv") #this is a DIFFERENT meta data file. Example in github. Include any information in this that you want to compare between samples
...
```{r}
data <- read.csv("~/Desktop/fixed_raw_counts.csv")
...
write.csv(counts, "~/Desktop/normalized_counts_OV90.csv")
```
...
```{r}
res_5 <- results(dds, contrast=c("condition","3.3","0"))

write.csv(res_5,"~/Desktop/P3_vs_P0.csv")
```

```{r}
res_6 <- results(dds, contrast=c("condition","4","3.3"))

write.csv(res_6,"~/Desktop/P3_vs_P4.csv")
```

```{r}
res_7 <- results(dds, contrast=c("condition","7","6.6"))

write.csv(res_7,"~/Desktop/P6_vs_P7.csv")
```
