...
ggplot(data = Three, aes(x = OV90, y = CAOV3, shape=factor(Hallmarks), fill=factor(Hallmarks), col=factor(Hallmarks))) + geom_point(alpha = 0.5, size=14, stroke=1.4) + scale_shape_manual(values=rep(c(21:25), times=10)) + geom_vline(xintercept = 0, 
                color = "black", linewidth=1.5) + geom_hline(yintercept=0, color = "black", linewidth=1.5) + theme(legend.key.size = unit(2.5, 'cm'), legend.text = element_text(size=25), legend.title=element_blank()) + guides(color=guide_legend(ncol=2)) + guides(shape=guide_legend(ncol=2)) 
