...
```{r}
# Keep only the significantly differentiated genes where the fold-change was at least 1.5 
sigGenes <- rownames(deseq2ResDF[deseq2ResDF$padj <= .05 & deseq2ResDF$log2FoldChange > 1.5, ])
sigGenes2 <- rownames(deseq2ResDF[deseq2ResDF$padj <= .05 & deseq2ResDF$log2FoldChange < -1.5, ])
sigGenes3 <- c(sigGenes, sigGenes2)
deseq2VST <- deseq2VST[deseq2VST$Gene %in% sigGenes3,]
```
