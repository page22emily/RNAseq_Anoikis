...
```{r}
# Keep only the significantly differentiated genes where the fold-change was at least 1.5 and we want to do the absolute value so that negative and postive are on the same figure 
sigGenes <- rownames(deseq2ResDF[deseq2ResDF$padj <= .05 & abs(deseq2ResDF$log2FoldChange) > 1.5, ])
...
```
```{r}
ggsave("~/Desktop/heatmap_up_and_down_regulated_genes_anoikis.tiff", plot = last_plot())
```