...
df$LC_1 <- foldchange(df$P01, df$counts_P0_average) 
df$LFC_41 <- foldchange2logratio(df$LC_1, base = 2)
df$LC_2 <- foldchange(df$P02, df$counts_P0_average)
df$LFC_42 <- foldchange2logratio(df$LC_2, base = 2)
df$LC_3 <- foldchange(df$P03, df$counts_P0_average)
df$LFC_43 <- foldchange2logratio(df$LC_3, base = 2)
```
...
df$LC_1 <- foldchange(df$P01, df$counts_P0_average) 
df$LFC_61 <- foldchange2logratio(df$LC_1, base = 2)
df$LC_2 <- foldchange(df$P02, df$counts_P0_average)
df$LFC_62 <- foldchange2logratio(df$LC_2, base = 2)
df$LC_3 <- foldchange(df$P03, df$counts_P0_average)
df$LFC_63 <- foldchange2logratio(df$LC_3, base = 2)
```
...
df$LC_1 <- foldchange(df$P01, df$counts_P0_average) 
df$LFC_71 <- foldchange2logratio(df$LC_1, base = 2)
df$LC_2 <- foldchange(df$P02, df$counts_P0_average)
df$LFC_72 <- foldchange2logratio(df$LC_2, base = 2)
df$LC_3 <- foldchange(df$P03, df$counts_P0_average)
df$LFC_73 <- foldchange2logratio(df$LC_3, base = 2)
```
...
```{r}
heatmap <- ggplot(combined, aes(x= variable, y=gene, fill=value)) + geom_raster() + scale_fill_viridis(discrete= FALSE) + theme(axis.text.x=element_text(angle=65, hjust=1), axis.text.y=element_blank(), axis.ticks.y=element_blank()) + aes(y = reorder(gene, value))
heatmap
```
