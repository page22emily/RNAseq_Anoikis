...
```{r}
P0_P1 <- read.csv("~/Documents/Mythreye_Lab/Anoikis_Project/Bulk_RNA/CAOV3/DESeq2/results_1_vs_0_CAOV3.csv")
P0_P6 <- read.csv("~/Documents/Mythreye_Lab/Anoikis_Project/Bulk_RNA/CAOV3/DESeq2/results_6_vs_0_CAOV3.csv")
P0_P7 <- read.csv("~/Documents/Mythreye_Lab/Anoikis_Project/Bulk_RNA/CAOV3/DESeq2/results_7_vs_0_CAOV3.csv")
...
```{r}
sigGenes <- as.list(c(P0_P1_adj2$X, P0_P1_adj3$X, P0_P7_adj2$X, P0_P7_adj3$X, P0_P6_adj2$X, P0_P6_adj3$X))
```
...
```{r}
ggsave("~/Desktop/CAOV3_P0_vs_P1_separated_heatmap.png", plot = last_plot(), width = 10, height = 12, units = c("in"))
```
...
```{r}
ggsave("~/Desktop/CAOV3_P0_vs_P6_separated_heatmap.png", plot = last_plot(), width = 10, height = 12, units = c("in"))
```
...
```{r}
ggsave("~/Desktop/CAOV3_P0_vs_P7_separated_heatmap.png", plot = last_plot(), width = 10, height = 12, units = c("in"))
```

```{r}
combined_CAOV3 <- rbind(de21, de2, de22, de23)
```
...
```
```{r}
ggsave("~/Desktop/CAOV3_separated_heatmap_multiple_tp.png", plot = last_plot(), width = 10, height = 12, units = c("in"))
```

```{r}
combined3 <- merge(deseq2VST_wide01, deseq2VST_wide1, all.x=TRUE, all= TRUE)
combined2 <- merge(combined3, deseq2VST_wide6, all.x=TRUE, all= TRUE)
combined <- merge(combined2, deseq2VST_wide7, all.x=TRUE, all= TRUE)
```

```{r}
write.csv(combined, "~/Desktop/combined_CAOV3.csv")
```


```{r}
library(readr)
combined_redo <- read_csv("~/Desktop/combined_CAOV3.csv", 
    col_types = cols(...1 = col_number(), 
        gene = col_number(), P1A = col_number(), 
        P1B = col_number(), P1C = col_number(), 
        P6A = col_number(), P6B = col_number(), P6C = col_number(),P7A = col_number(), P7B = col_number(), P7C = col_number()))
mat <- combined_redo[  , -2]
```
...
pdf("~/Desktop/heatmap_CAOV3_edits_2.pdf",width=8, height=11)
